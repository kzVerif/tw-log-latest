...
export async function truewalletTopup(inputValue: string): Promise<string | null> {
    const voucherCode = inputValue.replace('https://gift.truemoney.com/campaign/?v=', '');
  
    try {
      const response = await fetch(`https://api.xpluem.com/${voucherCode}/0957148911`);
      ...
      console.error('Error fetching TrueMoney top-up:', error);
      return null;
    }
  }
